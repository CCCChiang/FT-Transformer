...
from _utils import calibrate_mean_var
    ...
    class Head(nn.Module):
        ...
        def __init__(
            ...
            *,
            ...
            if self.fds:
                self.FDS = FDS(
                    feature_dim=d_in, bucket_num=self.bucket_num, bucket_start=self.bucket_start,
                    start_update=self.start_update, start_smooth=self.start_smooth, kernel=self.kernel,
                    ks=self.ks, sigma=self.sigma, momentum=self.momentum)

            self.normalization = _make_nn_module(normalization, d_in)
            self.activation = _make_nn_module(activation)
            self.linear = nn.Linear(d_in, d_out, bias)

        def forward(self, x: Tensor, targets=None, epoch=None) -> Tensor:
            x = x[:, -1]
            x = self.normalization(x)
            ...
            if self.fds:
                ...
                if self.training and self.fds:
                    # print(self.training, self.fds, epoch, self.start_smooth)
                    if epoch >= self.start_smooth:
...
class FTTransformer(nn.Module):
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor], targets=None, epoch=None) -> Tensor:
        x = self.feature_tokenizer(x_num, x_cat)
        x = self.cls_token(x)
        x = self.transformer(x, targets, epoch)
        return x
